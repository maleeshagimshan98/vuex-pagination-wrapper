const vuexPaginationWrapper = require("../src/vuexPaginationWrapper");
...
describe("vuexPaginationWrapper Class", () => {
  ...
  test("should initialize with default values", () => {
    ...
    expect(paginationWrapper.data).toEqual({});
    expect(paginationWrapper.getPageDataObject()).toEqual({ total: 1, current: 1, pages: 1 });
  });

  test("should initialize with provided data and page", () => {
    const data = { 1: "page1 data" };
    const page = { total: 10, current: 2, pages: 5 };
    paginationWrapper = new vuexPaginationWrapper(data, page);
    expect(paginationWrapper.data).toEqual(data);
    expect(paginationWrapper.getPageDataObject()).toEqual(page);
  });

  // 2. _initialisePage
  test("should initialize the Page object correctly", () => {
    paginationWrapper = new vuexPaginationWrapper(null, { total: 20, current: 5, pages: 10 });
    expect(paginationWrapper.page).toBeInstanceOf(Page);
    expect(paginationWrapper.getPageDataObject()).toEqual({ total: 20, current: 5, pages: 10 });
  });

  test("should create Page with default values if no page data is provided", () => {
    paginationWrapper = new vuexPaginationWrapper();
    expect(paginationWrapper.page.getPage()).toEqual({ total: 1, current: 1, pages: 1 });
  });

  // 3. getPageDataObject
  test("should return correct pagination data", () => {
    const page = { total: 10, current: 3, pages: 5 };
    paginationWrapper = new vuexPaginationWrapper(null, page);
    expect(paginationWrapper.getPageDataObject()).toEqual(page);
  });
  ...
  test("should return null if Page object is not initialized", () => {
    paginationWrapper = new vuexPaginationWrapper();
  ...
  // 6. _setPage
  test("should set the entire page object", () => {
    const page = { total: 10, current: 3, pages: 5 };
    paginationWrapper = new vuexPaginationWrapper();
    paginationWrapper._setPage(page);
    expect(paginationWrapper.getPageDataObject()).toEqual(page);
  });
  ...
  test("should return self after setting data", () => {
    const data = { item1: "value1" };
    paginationWrapper = new vuexPaginationWrapper();
    const result = paginationWrapper.setData(data, { total: 10, current: 2, pages: 5 });
  ...
  test("should return true if data exists for the specified page", () => {
    ...
    paginationWrapper = new vuexPaginationWrapper(data, { total: 10, current: 2, pages: 5 });
  ...
  test("should return the data for the specified page number", () => {
    const data = { 1: { item1: "value1" } };
    paginationWrapper = new vuexPaginationWrapper(data, { total: 10, current: 1, pages: 5 });
    expect(paginationWrapper.getData(1)).toEqual({ item1: "value1" });
  });

  test("should throw error if page number exceeds total pages", () => {
    paginationWrapper = new vuexPaginationWrapper(null, { total: 10, current: 1, pages: 5 });
    expect(() => paginationWrapper.getData(6)).toThrow("Invalid page number, Page number must be equal or lower than 5");
  });
});
